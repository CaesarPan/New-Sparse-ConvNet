...
model = SCN_Model(x_image, y_, keep_prob2, keep_prob3, keep_prob4, keep_prob5, keep_prob6)
...
test_prediction = np.zeros(shape=(len(test_x), _CLASS_NUM), dtype=np.float32)
...
for epoch in range(_EPOCH_NUM):
    ...
    for step in range(train_x.shape[0]/_BATCH_SIZE):
        ...
        if step % 100 == 0:
            train_accuracy = sess.run(model.accuracy, feed_dict={x: batch_x, y_: batch_y, keep_prob2: 1.0, keep_prob3: 1.0,
        ...
        sess.run(model.optimize, feed_dict={x: batch_x, y_: batch_y, keep_prob2: 0.9, keep_prob3: 0.8,
    ...
    for i in range(len(test_x) / _BATCH_SIZE):
        ...
        test_prediction[i * _BATCH_SIZE:(i + 1) * _BATCH_SIZE - 1, :] = sess.run(model.prediction,
                                                                              feed_dict={x: test_batch_x, y_: test_batch_y, keep_prob2: 1.0,
                                                                                         keep_prob3: 1.0, keep_prob4: 1.0, keep_prob5: 1.0, keep_prob6: 1.0})

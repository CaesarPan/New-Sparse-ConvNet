...
train_step = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.99, use_nesterov=True).minimize(cross_entropy)    # lr = 0.003*exp(-0.01*epoch)
...
for i in range(300000):  # Run 300 epochs
    batch_i_x = train_x[(i*_BATCH_SIZE) % train_x.shape[0]:((i+1)*_BATCH_SIZE-1) % train_x.shape[0]]
    batch_i_y = train_y[(i*_BATCH_SIZE) % train_x.shape[0]:((i+1)*_BATCH_SIZE-1) % train_x.shape[0]]
    ...
    if i % 100 == 0:
        ...
        if (i >= 290000) and (i % 1000 == 0):           # Only save models for the last 50 epochs
            saver.save(sess, save_path=_MODEL_SAVE_PATH, global_step=i)
